"""Cableado de DKOps para el caso de uso cdf.
...
def resolve_bundle_root(bundle_root: str | None = None) -> Path:
    ...
    if env:
        return Path(env)
    # Ejecución desde el repo: .../use_cases/cdf/src/orders/pipeline.py
...
def build_engine(
    ...
    engine = IngestionEngine(
        ...
        ops=IngestionOpsLogger(launcher.spark, ops_path=OPS_PATH, pipeline="orders"),
    )
